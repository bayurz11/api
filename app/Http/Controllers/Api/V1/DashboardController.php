...
class DashboardController extends Controller
{
    ...
    public function __invoke(Request $request): JsonResponse
    {
        try {
            ...
            Log::error('Dashboard analytics failed; serving operational fallback.', [
    ...
    /**
     * Keep the operational dashboard usable when an optional analytics table or
     * column has not been migrated on a deployment yet.
     *
     * @return array<string, mixed>
     */
    private function fallbackPayload(): array
    {
        $metric = static function (callable $query, int|float $default = 0): int|float {
            try {
                return $query();
            } catch (Throwable) {
                return $default;
        ...
        return [
            'summary' => [
                ...
                'ready_to_pay_bills' => $readyToPayBills,
                'today_sales' => $todaySales,
                'today_bills' => 0,
                'average_bill' => 0,
                'sales_growth_percent' => 0,
                'today_purchase_total' => 0,
                'today_estimated_cogs' => 0,
